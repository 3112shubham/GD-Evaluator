...
export default function AddTrainer() {
  ...
  const addTrainer = async () => {
    if (!newTrainer.name || !newTrainer.email) {
      setMessage({ type: 'error', text: 'Please enter trainer name and email' });
    ...
    try {
      ...
      if (emailExists) {
        setMessage({ type: 'error', text: 'A trainer with this email already exists' });
        setIsAdding(false);
        setTimeout(() => setMessage(null), 5000);
        return;
      }
      ...
      setMessage({ type: 'success', text: `Trainer added! Password reset sent to ${newTrainer.email}` });
      ...
      setMessage({ type: 'error', text: `${errorMessage}: ${err.message}` });
      setTimeout(() => setMessage(null), 5000);
    } finally {
      setIsAdding(false);
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
        ...
        <button
          onClick={addTrainer}
          disabled={!newTrainer.name || !newTrainer.email || isAdding}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg disabled:bg-gray-400 flex items-center justify-center gap-2"
        >
          {isAdding ? (
            <>
