...
import { collection, query, where, onSnapshot, updateDoc, doc, deleteDoc, orderBy, limit, startAfter, getDocs } from 'firebase/firestore';
...
export default function Dashboard({ hideNavbar = false, onViewEvaluations = null }) {
  const [activeGDs, setActiveGDs] = useState([]);
  const [pastGDs, setPastGDs] = useState([]);
  const [displayedPastGDs, setDisplayedPastGDs] = useState([]);
  const [totalPastGDCount, setTotalPastGDCount] = useState(0);
  const [hasMorePastGDs, setHasMorePastGDs] = useState(false);
  const [pastGDPage, setPastGDPage] = useState(0);
  const [pastGDHasMore, setPastGDHasMore] = useState(false);
  const [lastPastGDDoc, setLastPastGDDoc] = useState(null);
  const [loading, setLoading] = useState(true);
  const [deletingId, setDeletingId] = useState(null);
  const navigate = useNavigate();
  const PAST_GD_PAGE_SIZE = 6;
  ...
  useEffect(() => {
    ...
    // Fetch initial batch of past GDs with limit
    const fetchInitialPastGDs = async () => {
      try {
        ...
        const q = query(
          ...
          limit(PAST_GD_PAGE_SIZE + 1)
        );
        const snapshot = await getDocs(q);
        const docs = snapshot.docs;
        const hasMore = docs.length > PAST_GD_PAGE_SIZE;
        const docsToShow = hasMore ? docs.slice(0, PAST_GD_PAGE_SIZE) : docs;
        const gds = docsToShow.map(doc => ({
          id: doc.id,
          ...doc.data(),
          date: doc.data().createdAt?.toDate().toLocaleDateString(),
          completedDate: doc.data().completedAt?.toDate().toLocaleDateString(),
          completedAtTime: doc.data().completedAt?.toDate?.() || new Date()
        }));
        setPastGDs(gds);
        setDisplayedPastGDs(gds);
        setPastGDPage(0);
        setPastGDHasMore(hasMore);
        setHasMorePastGDs(hasMore);
        // Set total count: show actual count if 6 or less, show "6+" if more
        setTotalPastGDCount(hasMore ? `${PAST_GD_PAGE_SIZE}+` : gds.length);
        if (gds.length > 0) {
          setLastPastGDDoc(docsToShow[docsToShow.length - 1]);
        }
      } catch (err) {
  ...
  const loadMorePastGDs = async () => {
    if (!lastPastGDDoc || !pastGDHasMore) return;
    try {
      const q = query(
        collection(db, 'sessions'),
        where('type', '==', 'gd'),
      ...
      const hasMore = docs.length > PAST_GD_PAGE_SIZE;
      const docsToShow = hasMore ? docs.slice(0, PAST_GD_PAGE_SIZE) : docs;
      const newGDs = docsToShow.map(doc => ({
        id: doc.id,
        ...doc.data(),
        date: doc.data().createdAt?.toDate().toLocaleDateString(),
        completedDate: doc.data().completedAt?.toDate().toLocaleDateString(),
      ...
      setDisplayedPastGDs([...displayedPastGDs, ...newGDs]);
      setPastGDPage(pastGDPage + 1);
      setPastGDHasMore(hasMore);
      if (newGDs.length > 0) {
        setLastPastGDDoc(docsToShow[docsToShow.length - 1]);
      }
    } catch (err) {
      console.error('Error loading more past GDs:', err);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50">
      {/* Navbar - only show if not hidden by parent */}
      {!hideNavbar && (
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 rounded-xl shadow-md text-white flex flex-col justify-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold">{activeGDs.length}</div>
                <div className="text-sm">Active Sessions</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold">{totalPastGDCount}</div>
                ...
                { hasMorePastGDs && (
                  <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg text-center">
                    <p className="text-sm text-blue-700 mb-3">
                      Showing {displayedPastGDs.length} of <span className="font-semibold">{totalPastGDCount} completed</span> GD sessions
                    </p>
                    <button
                      onClick={onViewEvaluations}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-medium rounded-lg hover:from-blue-700 hover:to-indigo-800 transition"
                    >
                      Go to Evaluations for more
                    </button>
                  </div>
                )}
              </div>
